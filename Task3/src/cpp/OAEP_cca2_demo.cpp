...
auto parse_size(const std::string &value, const std::string &name) -> std::size_t {
...
auto main(int argc, char *argv[]) -> int {
	if (argc < 2 || argc > 8) {
		std::cerr << "Usage: " << argv[0]
				  << " <ciphertext_hex|\"Ciphertext: ciphertext_hex\"> "
					 "[k0_bits] [k1_bits] [public_key_file] [private_key_file] [hash]\n";
		return 1;
	}
	...
	const std::size_t k0 =
		argc > next_arg ? parse_size(argv[next_arg], "k0_bits") : DEFAULT_K0;
	const std::size_t k1 =
		argc > next_arg + 1 ? parse_size(argv[next_arg + 1], "k1_bits") : DEFAULT_K1;
	const std::string public_key_file =
		argc > next_arg + 2 ? argv[next_arg + 2] : "../../../Task1/RSA_Public_Key.txt";
	const std::string private_key_file =
		argc > next_arg + 3 ? argv[next_arg + 3] : "../../../Task1/RSA_Secret_Key.txt";
	const OAEP::HashAlgo hash =
		argc > next_arg + 4 ? parse_hash(argv[next_arg + 4]) : DEFAULT_HASH;
	...
	mpz_class s;
	for (s = 2;; ++s) {
		if (mpz_invert(s_inverse.get_mpz_t(), s.get_mpz_t(), n.get_mpz_t()) == 0)
			continue;
	...
	try {
		...
		std::cout << "OAEP decryption rejected forged ciphertext: " << ex.what() << '\n'
