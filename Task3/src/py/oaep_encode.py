#!/usr/bin/env python3
"""RSA-OAEP 加密（教学版，默认 k0=512 k1=64 SHA-512）。

用法:  python oaep_encode.py <plaintext> [k0_bits] [k1_bits] [public_key_file]

复用 Task1 的 RSA 公钥。输出（写到 Task3/ 目录）:
  Random_Number.txt        : OAEP 随机数 r（k0 位，128 hex）
  Message_After_Padding.txt: 编码块 EM = X || Y（n 位，256 hex）
  Encrypted_Message.txt    : RSA-OAEP 密文（n 位，256 hex）
"""
